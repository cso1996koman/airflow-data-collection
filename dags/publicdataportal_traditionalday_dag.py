...
class PublicDataPortalTraditionalDayDag:
    @staticmethod
    def create_publicdataportal_traditionalday_dag(dag_config_param : dict, dag_id : str, schedule_interval : timedelta, start_date : datetime, default_args : dict) -> DAG:
        @dag(dag_id=dag_id,
                    schedule_interval=schedule_interval,
                    params=dag_config_param,
                    start_date=start_date,
                    default_args=default_args)
        def publicdataportal_traditionalday_dag() -> DAG:
            @task
            def open_api_request():
                ...
                request_url = None
                publicdataportal_anniversary_url_obj : PublicDataPortalTraditionalDayUrl = None
                if prev_task_instance is None:
                    request_url = dag_config_param['uri']
                    assert request_url is not None
                    publicdataportal_anniversary_url_obj : PublicDataPortalTraditionalDayUrl = UrlObjectFactory.createPublicDataPortalTraditionalDayUrl(request_url)
                ...
                request_url_obj : PublicDataPortalTraditionalDayUrl = UrlObjectFactory.createPublicDataPortalTraditionalDayUrl(request_url)
                next_request_url_obj : PublicDataPortalTraditionalDayUrl = UrlObjectFactory.createPublicDataPortalTraditionalDayUrl(request_url)
            ...
            def open_api_csv_save():
                ...
                xcom_dto : OpenApiXcomDto = prev_task_instance.xcom_pull(key=f"{dag_id}_open_api_request_{prev_task_instance.run_id}")
            ...
            def open_api_hdfs_save():
                ...
                xcom_dto : OpenApiXcomDto = prev_task_instance.xcom_pull(key=f"{dag_id}_open_api_csv_save_{prev_task_instance.run_id}")
                assert xcom_dto is not None
        ...
        return publicdataportal_traditionalday_dag()